...
class imprimeTicket:
	def imp(self,codigo,respuesta,cuantos,acuerdo,veces, cualpreg):
		...
		if cualpreg == 1:
			os.system('echo "La conciencia de ser observado aumenta" > /dev/ttyS0')
			os.system('echo "su sensacion de seguridad? " > /dev/ttyS0')
		
		if cualpreg == 2:
			os.system('echo "Estar en una base de datos " > /dev/ttyS0')
			os.system('echo "es pertenecer a una comunidad?" > /dev/ttyS0')
